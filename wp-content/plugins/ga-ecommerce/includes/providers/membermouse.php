...
class MonsterInsights_eCommerce_MemberMouse_Integration extends MonsterInsights_Enhanced_eCommerce_Integration {
	...
	public function get_transaction_meta( $transaction_id ) {
		...
		$row        = $wpdb->get_row( // phpcs:ignore WordPress.DB.DirectDatabaseQuery
			$wpdb->prepare( "SELECT * FROM %i WHERE transaction_id=%d", [$table_name, $transaction_id] ),
			ARRAY_A
		);
	...
	public function store_transaction_meta( $transaction_id, $order_id, $ga_uuid, $cookie, $monsterinsights_is_in_ga = null ) {
		...
		if ( $monsterinsights_is_in_ga == null ) {
			...
			$sql                      = $wpdb->prepare( $sql, $transaction_id, $ga_uuid, $order_id, $cookie, $ga_uuid, $cookie ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- SQL is prepared, but phpcs can't detect it
		} else {
			$monsterinsights_is_in_ga = boolval( $monsterinsights_is_in_ga ) ? 1 : 0;
			...
			$sql                      = $wpdb->prepare( $sql, $transaction_id, $ga_uuid, $order_id, $cookie, $monsterinsights_is_in_ga, $ga_uuid, $cookie, $monsterinsights_is_in_ga ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- SQL is prepared, but phpcs can't detect it
		}
		$res = $wpdb->query( $sql ); // phpcs:ignore WordPress.DB -- Direct database query is necessary here. Prepared statements handled above.
		if ( $res === false ) {
	...
	public function store_user_session_id( $transaction_id, $session_id ) {
		...
		return $wpdb->query( // phpcs:ignore WordPress.DB.DirectDatabaseQuery
			$wpdb->prepare( "INSERT INTO %i ( transaction_id, ga_session_id ) VALUES ( %d , %s ) ON DUPLICATE KEY UPDATE ga_session_id = %s", $table, $transaction_id, $session_id, $session_id )
		);
	}
	...
	public function get_user_session_id( $transaction_id ) {
		...
		return $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery
			$wpdb->prepare( "SELECT ga_session_id FROM %i WHERE transaction_id = %d", $table, $transaction_id )
		);
	}
}

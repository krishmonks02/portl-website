...
class MonsterInsights_eCommerce_EDD_Integration extends MonsterInsights_Enhanced_eCommerce_Integration {
	...
	private function hooks() {
		...
		add_action( 'monsterinsights_frontend_tracking_gtag_after_pageview', array( $this, 'product_detail' ), 10 );
		...
		// Checkout Page
		add_action( 'monsterinsights_frontend_tracking_gtag_after_pageview', array( $this, 'checkout_page' ), 10 );
	...
	public function impression( $download_id, $args ) {
		...
		if ( is_array( $categories ) && ! is_wp_error( $categories ) ) {
			foreach ( $categories as $category ) {
				if ( ! empty( $category ) && ! is_wp_error( $category ) && isset( $category->name ) ) {
	...
	public function product_detail() {
		...
		// Store the event data to be output later
		$event_data = array(
			'event' => 'view_item',
			'items' => array( $product_data ),
		);
		// Add to the queue for later output
		$this->enqueue_js( 'event', $this->get_event_js_string( $event_data ) );
	}
	...
	private function get_event_js_string( $event_data ) {
		ob_start();
		?>
		if (typeof __gtagTracker !== 'undefined' && __gtagTracker) {
			__gtagTracker('event', '<?php echo esc_js( $event_data['event'] ); ?>', {
				<?php if ( isset( $event_data['value'] ) ): ?>
				value: <?php echo esc_js( $event_data['value'] ); ?>,
				<?php endif; ?>
				<?php if ( isset( $event_data['currency'] ) ): ?>
				currency: '<?php echo esc_js( $event_data['currency'] ); ?>',
				<?php endif; ?>
				<?php if ( isset( $event_data['items'] ) ): ?>
				items: <?php echo wp_json_encode( $event_data['items'] ); ?>,
				<?php endif; ?>
				<?php if ( isset( $event_data['transaction_id'] ) ): ?>
				transaction_id: '<?php echo esc_js( $event_data['transaction_id'] ); ?>',
				<?php endif; ?>
				<?php if ( isset( $event_data['tax'] ) ): ?>
				tax: <?php echo esc_js( $event_data['tax'] ); ?>,
				<?php endif; ?>
				<?php if ( isset( $event_data['coupon'] ) ): ?>
				coupon: '<?php echo esc_js( $event_data['coupon'] ); ?>',
				<?php endif; ?>
				<?php if ( isset( $event_data['session_id'] ) ): ?>
				session_id: '<?php echo esc_js( $event_data['session_id'] ); ?>'
				<?php endif; ?>
			});
		}
		<?php
	...
	public function change_cart_quantity() {
		...
		if ( empty( $_POST['quantity'] ) || empty( $_POST['download_id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Not even EDD checks this
			return;
		}
		...
		// Get download ID
		$download = new EDD_Download( $_POST['download_id'] ); // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Not even EDD checks this

		// Let's see if this is for a variation
		$options = isset( $_POST['options'] ) ? maybe_unserialize( stripslashes( $_POST['options'] ) ) : array(); // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Not even EDD checks this

		$original_quantity = edd_get_cart_item_quantity( $download->ID, $options );
		$new_quantity      = absint( $_POST['quantity'] ); // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Not even EDD checks this
		...
		$download_id = sanitize_text_field( wp_unslash( $_POST['download_id'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Not even EDD checks this

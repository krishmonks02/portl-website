...
if ( ! class_exists( 'AWCDP_Email_Partial_Payment' ) ) {

	class AWCDP_Email_Partial_Payment extends WC_Email {

		public function __construct() {
			$this->id          = 'awcdp_partial_payment';
			$this->title       = __( 'Acowebs Partial Payment', 'deposits-partial-payments-for-woocommerce' );
			$this->description = __( 'This Emails will be sent when an order is partially paid', 'deposits-partial-payments-for-woocommerce' );

			$awwlm_es = get_option('woocommerce_awcdp_partial_payment_settings');
			$pp_subject = ( isset($awwlm_es['subject']) && $awwlm_es['subject'] != '' ) ? $awwlm_es['subject'] : __( '[{site_title}] Order partially paid ({order_number}) - {order_date}', 'deposits-partial-payments-for-woocommerce' );
			$pp_heading = ( isset($awwlm_es['heading']) && $awwlm_es['heading'] != '' ) ? $awwlm_es['heading'] : __( 'Order partially paid', 'deposits-partial-payments-for-woocommerce' );
			...
			*/
			add_action('woocommerce_order_status_completed_notification', array($this, 'check_trigger'));
		...
		function check_trigger($order_id, $order = false){
...
return new AWCDP_Email_Partial_Payment();

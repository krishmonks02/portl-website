<?php
/**
 * Customer Partial Payment email
 *
 */
...
if ( ! class_exists( 'AWCDP_Email_Partial_Paid' ) ) {

	class AWCDP_Email_Partial_Paid extends WC_Email {

		private $still_needs_payment = false;

		public function __construct() {
			$this->id          = 'awcdp_partial_paid';
			$this->title       = __( 'Acowebs Partial Payment Received', 'deposits-partial-payments-for-woocommerce' );
			$this->description = __( 'This emails will be sent to the customer when an order partial payment is received', 'deposits-partial-payments-for-woocommerce' );
      $this->customer_email = true;

			// $awwlm_es = get_option('woocommerce_awcdp_partial_paid_settings');
			//
			...
			$this->template_html  = 'emails/customer-order-partial-paid.php';
			$this->template_plain = 'emails/plain/customer-order-partial-paid.php';
		...
		function trigger( $order_id, $order = false ){
			...
			if (is_a($order, 'WC_Order')) {
				...
				if($order->get_type() != AWCDP_POST_TYPE || $order->get_meta('_awcdp_deposits_payment_type', true) == 'deposit'){
					return;
				}
				...
				$partial_payments = $this->awcdp_get_partial_payments($parent->get_id());

				if($partial_payments){
					foreach($partial_payments as $single_payment){
						if($single_payment->needs_payment()) {
							$this->still_needs_payment = true;
						}
					}
				}

				if(!$this->still_needs_payment) {
					return;
				}
		...
		function awcdp_get_partial_payments($order_id, $args = array(), $object = true){
			$default_args = array(
    	...
    	return esc_html__('Thank you for your payment', 'deposits-partial-payments-for-woocommerce');
    }
    	...
    	return esc_html__("Your payment {partial_payment_number} has been received.", 'deposits-partial-payments-for-woocommerce');
    }
...
return new AWCDP_Email_Partial_Paid();

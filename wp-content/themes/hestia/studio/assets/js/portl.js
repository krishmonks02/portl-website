...
// features_section studio
$(function () {
    if ($(".features_section").length) {
        // Only initialize Swiper on devices below 768px
        if (window.innerWidth < 1024) {
            const features_section = new Swiper(".features_section .swiper", {
                loop: false,
                slidesPerView: 'auto',
                spaceBetween: 40,
                speed: 600,
...
// Initialize mobile features grid swiper
document.addEventListener('DOMContentLoaded', () => {
    const mobileSliderContainer = document.querySelector('.portl_features_grid_mobile .features-grid-swiper');
    if (mobileSliderContainer && typeof Swiper !== 'undefined') {
        new Swiper(mobileSliderContainer, {
            slidesPerView: 1,
            spaceBetween: 12,
            slidesOffsetBefore: 24,
            slidesOffsetAfter: 24,
...
$(function () {
    if ($(".swiper_v3").length) {
        const swiperV3 = new Swiper(".swiper_v3 .swiper", {
            loop: false,
            slidesPerView: 'auto',
            autoHeight: true,
            spaceBetween: 0,
            ...
            navigation: {
                nextEl: '.swiper_v3 .swiper-button-next',
                prevEl: '.swiper_v3 .swiper-button-prev',
            },
        });
    }
})

// swiper_v4 - static initialization with synced pagination swiper
$(function () {
    if ($('.swiper_v4').length) {
        // Initialize main image swiper
        const swiperV4 = new Swiper('.swiper_v4 .portl_swiper_v4--container.swiper', {
            loop: false,
            slidesPerView: 1,
            spaceBetween: 0,
            freeMode: false,
        ...
        // Initialize pagination swiper
        if (window.innerWidth < 1024) {
            const paginationSwiper = new Swiper('.swiper_v4 .portl_swiper_v4--pagination', {
                slidesPerView: 'auto',
                freeMode: {
                    enabled: true,
                    sticky: true
                ...
                allowTouchMove: true,
                speed: 1000, //set via CSS variable below for onclick
            });
            // Setting speed for CSS variable to make smooth onClick + touchMove + mouseWheel
            document.querySelector(".swiper_v4 .swiper-wrapper").style.setProperty("--swv_speed", "1000ms");

            // Sync the two swipers (controller sync keeps visual state in sync)
            swiperV4.on('slideChange', () => {
                paginationSwiper.slideTo(swiperV4.activeIndex);
            });
            paginationSwiper.controller.control = swiperV4;

        } else {
            // On desktop, clicking pagination slides the main swiper
            $('.swiper_v4 .portl_swiper_v4--pagination .swiper-slide').on('click', function () {
                var index = $(this).index();
                swiperV4.slideTo(index);
                $('.swiper_v4 .portl_swiper_v4--pagination .swiper-slide').removeClass('swiper-slide-active');
                $(this).addClass('swiper-slide-active'); // Change to 'active' class
            });

            // Set the first slide as active by default
            $('.swiper_v4 .portl_swiper_v4--pagination .swiper-slide').first().addClass('swiper-slide-active');
        }
...
$(function () {
    if ($(".swiper_v5").length) {
        const swiperV5 = new Swiper('.swiper_v5 .swiper', {
            loop: false,
            slidesPerView: 1,
            spaceBetween: 40,
            speed: 600,
            ...
            navigation: {
                nextEl: '.swiper_v5 .swiper-button-next',
                prevEl: '.swiper_v5 .swiper-button-prev',
            },
        });
    }
})
...
$(function () {
    ...
    function isAnyModalOpen() {
        return $('.portl_modal.open').length > 0;
    }

    // Helper function to manage Lenis state based on modal status
    function updateLenisState() {
        if (!window.lenis) return;

        if (isAnyModalOpen()) {
            // Disable Lenis smooth scroll when any modal is open
            if (typeof window.lenis.stop === 'function') {
                window.lenis.stop();
            }
        } else {
            // Re-enable Lenis smooth scroll when all modals are closed
...
$(function () {
    ...
    $('.portl_header_stikcy--list a').on('click', function (event) {
        ...
        var offset = 0;
        if (!$('main').hasClass('withStickyHeader')) {
            offset = 64;
        }
        $('html, body').animate({
    ...
    // This code closes the sticky menu after link click & scroll on mobile (<1024px)
    $(function () {
        if ($(window).width() < 1024) {
            $('.portl_header_stikcy--list a').click(function (e) {
                const anchor = $(this);

                // If this click is triggered by our timeout, ignore it
                if (anchor.data('second-click')) {
                    anchor.data('second-click', false); // reset flag
                    return; // prevent looping
                }

                // Schedule the second click after 300ms
                const timeoutId = setTimeout(() => {
                    anchor.data('second-click', true); // mark as "programmatic click"
                    anchor.trigger('click'); // trigger second click
                }, 500);
            });
